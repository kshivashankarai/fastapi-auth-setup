...
class Settings(BaseSettings):
    # App
    APP_NAME: str = os.environ.get("APP_NAME", "FastAPI-Auth")
    ...
    MYSQL_USER: str = os.environ.get("MYSQL_USER", "shiva")
    MYSQL_PASS: str = os.environ.get("MYSQL_PASSWORD", "123456")
    MYSQL_PORT: int = int(os.environ.get("MYSQL_PORT", 3306))
    MYSQL_DB: str = os.environ.get("MYSQL_DB", "fastapi_auth_setup")
    DATABASE_URI: str = (
        f"mysql+pymysql://{MYSQL_USER}:%s@{MYSQL_HOST}:{MYSQL_PORT}/{MYSQL_DB}"
        % quote_plus(MYSQL_PASS)
    )
